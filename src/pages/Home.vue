  ...
  <div class="book-list-age">
    <app-form>
      ...
      <template v-slot:area>
        <div class="grid xl:grid-cols-6 lg:grid-cols-4 sm:grid-cols-2 gap-4 p-6 min-h-12">
          <div v-for="(book, idx) in data.bookLists" :key="`book-list-${idx}`">
            <router-link :to="`/books/${book.id}`">
              <div class="book-card rounded-md bg-neutral-100 h-72 p-6 hover:shadow-lg hover:scale-90 cursor-pointer">
                <div class="flex justify-center">
                  <img class="w-24 h-24 my-3" :src="require('@/assets/img/book-stack.png')">
                </div>
